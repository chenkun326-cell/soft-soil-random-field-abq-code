...
# 获取 para1 和 para2 的形状（大小）
para1_shape = para1.shape
para2_shape = para2.shape
...
for inp in range(numinp):
    for ele in elelist:
        ...
        # 检查索引是否在范围内
        if num < para1_shape[0] and inp < para1_shape[1]:
            ...
            mdb.models[Modelname].materials['TempM-' + str(ele)].MohrCoulombPlasticity(table=((para4[num][inp], 0.0),))
